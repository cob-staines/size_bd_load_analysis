...
# setup
librarian::shelf(tidyverse, dbplyr, here, janitor, RPostgres, DBI, RIBBiTR-BII/ribbitrrr, brms, tidybayes, ggplot2, bayesplot, runner, cmdstanr, bayestestR)
...
m5c_zln <- brm(
  formula = bf(
    ...
    hu ~ 1 + life_stage_simple * taxon_capture + precip_mm_15_z + precip_mm_15_z2 + temp_c_15_z + temp_c_15_z2 + (1 | population) + (1 + precip_mm_30_z + precip_mm_30_z2 + temp_c_30_z + temp_c_30_z2 | taxon_capture) + (1 | population:taxon_capture:life_stage_simple),
    sigma ~ 1 + taxon_capture +  (1 | population) + (1 | population:taxon_capture)
  ),
  data = brms_data,
  family = hurdle_lognormal(),
...
m5g_zln <- brm(
  formula = bf(
    bd_load_co_density ~ 1 + life_stage_simple * taxon_capture + precip_mm_30_z + precip_mm_30_z2 + temp_c_30_z + temp_c_30_z2 + (1 | population) + (1 + precip_mm_30_z + precip_mm_30_z2 + temp_c_30_z + temp_c_30_z2 | taxon_capture:life_stage_simple) + (1 | population:taxon_capture:life_stage_simple),
    hu ~ 1 + life_stage_simple * taxon_capture + precip_mm_15_z + precip_mm_15_z2 + temp_c_15_z + temp_c_15_z2 + (1 | population) + (1 + precip_mm_30_z + precip_mm_30_z2 + temp_c_30_z + temp_c_30_z2 | taxon_capture:life_stage_simple) + (1 | population:taxon_capture:life_stage_simple),
    sigma ~ 1 + taxon_capture +  (1 | population) + (1 | population:taxon_capture)
  ),
  data = brms_data,
  family = hurdle_lognormal(),
...
saveRDS(m5g_zln, file = "unified_models/unified_model_results_zln_life_tax_poptls2env2_env2_sigls.rds")
m5g_zln = readRDS(file = "unified_models/unified_model_results_zln_life_tax_poptls2env2_env2_sigls.rds")
...
pp_check(m5f_zln, type = "dens_overlay", prefix = "ppc", ndraws = 100) +
  scale_x_log10()
```
...
# prevalence
hu_taxon_subadult = spread_draws(m5f_zln, `b_hu_life_stage_simplesubadult.*`, regex = TRUE) %>%
...
hu_taxon_subadult_rope = hu_taxon_subadult %>%
  mutate(in_rope = abs(hu_subadult) <= 0.18) %>%
  group_by(species) %>%
  summarize(n = n(),
...
# load
b_taxon_subadult = spread_draws(m5f_zln, `b_life_stage_simplesubadult.*`, regex = TRUE) %>%
...
climate_draws = spread_draws(m5f_zln, `b_temp_c.*|b_precip_mm.*|b_hu_temp_c.*|b_hu_precip_mm.*|b_Intercept|b_hu_Intercept`, regex = TRUE)
