...
# setup
librarian::shelf(tidyverse, dbplyr, here, janitor, RPostgres, DBI, RIBBiTR-BII/ribbitrrr, brms, tidybayes, ggplot2, ggrepel, bayesplot, runner, cmdstanr, bayestestR, ropensci/rotl, ggtree, ape, patchwork, viridis, terra)
...
# csv data
##  RACA data
raca_data = read.csv(here("data", "JPSL_RACA_Swabbed_Individuals_2026-03-03.csv")) %>%
  clean_names() %>%
  filter(!(life_stage %in% c("L", "M"))) %>%
...
raca_population_valid = raca_data %>%
  group_by(population) %>%
...
peace = compare_df_cols(raca_valid,
                        usgs_valid,
                        ribbitr_valid)

data_combined = bind_rows(raca_valid,
                      usgs_valid,
                      ribbitr_valid) %>%
...
bd_model = data_combined %>%
  filter(taxon_capture %in% taxa_valid$taxon_capture)
...
# chunk_date_range = function(start_date, end_date, max_years) {
...
write_csv(site_space_time, here("python", "gee_env_pull", "data", paste0("site_coords_dates_all_", today(), ".csv")))
write_csv(site_space_time_before_2017, here("python", "gee_env_pull", "data", paste0("site_coords_dates_through_2016_", today(), ".csv")))
write_csv(site_space_time_after_2017, here("python", "gee_env_pull", "data", paste0("site_coords_dates_2017_beyond_", today(), ".csv")))
write_csv(head(site_space_time_after_2017), here("python", "gee_env_pull", "data", paste0("site_coords_dates_2017_beyond_", today(), "_test.csv")))
...
env_in = read_csv(here("python", "gee_env_pull", "data", "all_sites_combined.csv"))

peace = env_in %>%
  group_by(site_name) %>%
  summarize(n_precip = sum(!is.na(chirps)),
            n_tmin = sum(!is.na(chirts_tmin)),
            n_tmax = sum(!is.na(chirts_tmax)),
            n_rh = sum(!is.na(chirts_rh)),
            .groups = "drop")

meace = peace %>%
  filter(n_precip > n_tmax)
...
agg_species_stats = bd_model_env_select_stats %>%
  group_by() %>%
  summarize(med_sites = median(site_n),
            med_years = median(year_count),
            med_adult = median(adult_n),
            med_sub = median(subadult_n),
            sum_adult = sum(adult_n),
            sum_sub = sum(subadult_n),
            .groups = "drop")

add_visit_stats = bd_model_env_select %>%
  select(site, date) %>%
  distinct() %>%
  nrow()

# write_csv(bd_model_env_select_stats, here("clh", "species_data_stats.csv"))

# export site coords
# all_sites = unique(bd_model_env_select$site)
# 
# select_sites = db_site %>%
...
#   arrange(site)
# 
# write.csv(select_sites, here("data", paste0("bd_model_unified_select_sites", today(), ".csv")))

# write.csv(bd_model_env_select_stats, here("data", paste0("bd_model_unified_select_stats", today(), ".csv")))

# spp_climate_stats = bd_model_env_select %>%
#   select(taxon_capture,
#          site) %>%
#   distinct() %>%
#   left_join(site_climate, by = "site") %>%
#   group_by(taxon_capture) %>%
#   summarize(mean_annual_precip_mm = mean(site_precip_mm_annual_mean),
#             mean_annual_temp_c = mean(site_temp_c_mean))
...
# break out rona sierrae independently
yrep = posterior_predict(model_select,
                         ndraws = 1000)
ind_rs = which(brms_data$taxon_capture == "rana_sierrae")
ind_rs_not = which(brms_data$taxon_capture != "rana_sierrae")
y_rs = brms_data$bd_load_co_density[ind_rs]
y_rs_not = brms_data$bd_load_co_density[ind_rs_not]

yrep_rs = yrep[, ind_rs]
yrep_rs_not = yrep[, ind_rs_not]

y_combined = c(y_rs, y_rs_not)
yrep_combined = cbind(yrep_rs, yrep_rs_not)
group_labels = c(rep("rana_sierrae", length(y_rs)),
                 rep("rana_sierrae_not", length(y_rs_not)))
...
# break our rana sierrae into life stage
ind_adult = which(brms_data$taxon_capture == "rana_sierrae" & brms_data$life_stage_simple == "adult")
ind_sub = which(brms_data$taxon_capture == "rana_sierrae" & brms_data$life_stage_simple == "subadult")
y_adult = brms_data$bd_load_co_density[ind_adult]
y_sub = brms_data$bd_load_co_density[ind_sub]

yrep_adult = yrep[, ind_adult]
yrep_sub = yrep[, ind_sub]

y_combined = c(y_adult, y_sub)
yrep_combined = cbind(yrep_adult, yrep_sub)
group_labels = c(rep("rana_sirrae_adult", length(y_adult)),
                 rep("rana_sierrae_subadult", length(y_sub)))
...
b_hu = spread_draws(model_select, `b_hu.*`, regex = TRUE)

median_b_hu = median(b_hu$b_hu_Intercept)
median_b_hu_p = plogis(median_b_hu)
hu_p_upper = median_b_hu_p + 0.05
hu_p_lower = median_b_hu_p - 0.05
hu_q_upper = qlogis(hu_p_upper)
hu_q_lower = qlogis(hu_p_lower)
hu_scale = (hu_q_upper - hu_q_lower) / 2
