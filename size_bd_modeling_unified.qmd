...
b_taxon_subadult = spread_draws(model_select, `b_life_stage_simplesubadult.*`, regex = TRUE) %>%
  ...
  mutate(taxon_capture = gsub("b_life_stage_simplesubadult:taxon_capture", "", parameter),
         ...
         species = get_species_name(taxon_capture)) %>%
  mutate(b_subadult_sc = b_subadult / median_b_sigma)
...
p_b_dist = ggplot(b_taxon_subadult, aes(x = b_subadult)) +
  ...
  labs(
       ...
       x = "Bd Load Subadult Effect Size (log scale) [-]",
       y = "Posterior Density [-]") +
  theme(text=element_text(size=14))
...
p_ppos = ggplot(taxon_subadult_rope, aes(x = ratio_positive_hu, y = ratio_positive_b, color = species)) +
...
# no text
p_ppos_no_text = ggplot(taxon_subadult_rope, aes(x = ratio_positive_hu, y = ratio_positive_b, color = species)) +
  geom_vline(xintercept = 0.5, color = "#aaa") +
  geom_hline(yintercept = 0.5, color = "#aaa") + 
  geom_point(size = 4) +
  xlim(0, 1) +
...
# median
p_b_hu_median = ggplot(taxon_subadult_rope, aes(x = hu_median, y = b_median, color = species)) +
  geom_vline(xintercept = 0, color = "#aaa") +
  geom_hline(yintercept = 0, color = "#aaa") + 
  geom_point(size = 4) +
...
# median no text
p_b_hu_median_no_text = ggplot(taxon_subadult_rope, aes(x = hu_median, y = b_median, color = species)) +
  geom_vline(xintercept = 0, color = "#aaa") +
  geom_hline(yintercept = 0, color = "#aaa") + 
  geom_point(size = 4) +
  xlim(-1.6, 1.6) +
...
ggplot(taxon_subadult_rope, aes(x = hu_median, y = b_median, color = species)) +
  ...
  geom_text_repel(aes(label = species)) +
  theme_minimal() +
  labs(title = "Parameter Value Distributions",
       x = "Bd Prevalence (logit scale)",
       y = "Median Bd Load (log scale)")
...
# cophen_dist_ordered = 
hu_order_m = factor(gsub(" ", "_", hu_order), levels = gsub(" ", "_", hu_order))
b_order_m = factor(gsub(" ", "_", b_order), levels = gsub(" ", "_", b_order))
cophen_dist_ranked = cophen_dist[b_order_m, hu_order_m]
