...
bd_capture_valid = bd_population %>%
  inner_join(pop_year_valid, by = c("population", "year"), copy = TRUE)
...
pop_year_tx = pop_year %>%
  ...
  mutate(release_event = if_else(is.na(release_event), FALSE, release_event))
...
pop_adult_swab_counts = bd_stats %>%
  group_by(population) %>%
  summarise(adult_swab_n = sum(adult_swab_n))

release_event_lines <- pop_year_bd %>%
  filter(release_event) %>%
  select(population, year)
...
ggplot(pop_year_bd, aes(x = year)) +
  ...
  geom_point(aes(y = adult_bd_prevalence_ci_lower * 10, color = "Mean Load")) +
  geom_line(aes(y = adult_bd_prevalence_ci_lower * 10, color = "Mean Load")) +
  geom_vline(data = release_event_lines, aes(xintercept = year), linetype = "dashed") +
...
epi_class = pop_year_bd %>%
  ...
  rowwise() %>%
  mutate(transient = mean(c_across(c(next1, next2, next3)), na.rm = TRUE) < 0.1) %>%
  ungroup() %>%
  arrange(population, year) %>%
