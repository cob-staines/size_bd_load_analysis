...
data_pop = read.csv(here("data", "sn_site_pop_2_2025-10-31.csv")) %>%
  ...
  mutate(reintro_status = case_when(
    population %in% augmentations ~ "augmentation",
    population %in% reintroductions ~ "reintroduction",
    .default = "none"
  ))
...
first_treatment_events =  treatment_events %>%
  group_by(population) %>%
  summarise(first_treatment_year = min(year),
            .groups = "drop")

post_treatment_mask_sites = c()  # currently not masking any events
...
post_treatment_mask = expand_grid(unique(first_treatment_events$population),
                       seq(min(first_treatment_events$first_treatment_year), 2025)) %>%
  rename(population = 1,
         year = 2) %>%
  left_join(first_treatment_events, by = "population") %>%
  filter(year >= first_treatment_year) %>%
  select(-first_treatment_year) %>%
  filter(population %in% post_treatment_mask_sites)
...
bd_capture_valid = bd_population %>%
  ...
  anti_join(post_treatment_mask, by = c("population", "year"), copy = TRUE)
