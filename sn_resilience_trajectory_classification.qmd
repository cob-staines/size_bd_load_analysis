...
# adult bd prevalence threshold: 0.8
# adult median bd load threshold: 15000
...
# define epi events as consecutive years over bd prevalence/load threshold
## calculate ves drop as difference in median pre-and post-ves for each event
## epizootic is first event where ves drop >= 30
## if no epizootic identified: if median prevalence < 50 ~ naive, otherwise post
...
pop_year_class = pop_year_bd %>%
  ...
  rowwise() %>%
  mutate(
    low_count = all(c_across(c(adult_count_visual, adult_count_cmr)) < 20, na.rm = TRUE),
    low_count = if_else(is.na(adult_count_visual) & is.na(adult_count_cmr), NA, low_count)
  ) %>%
  ungroup() %>%
  group_by(population) %>%
  mutate(
    median_prev = median(adult_bd_prevalence, na.rm = TRUE),
    first_low_count_year_after_epi = min(year[year >= end_year & low_count], na.rm = TRUE),
    first_low_count_year_after_epi = if_else(first_low_count_year_after_epi > end_year, first_low_count_year_after_epi - 1, first_low_count_year_after_epi),
    first_low_count_year_after_epi = if_else(first_low_count_year_after_epi == Inf, end_year, first_low_count_year_after_epi)
  ) %>%
  ungroup() %>%
  mutate(
    res_class = case_when(
      ...
      year >= start_year & year <= first_low_count_year_after_epi ~ "2-epizootic",
      year > start_year ~ "3-post"),
    res_class_post = case_when(
      res_class == "1-naive" ~ "1-naive",
      res_class == "2-epizootic" ~ "2-epizootic",
      res_class == "3-post" & (adult_count_visual >= 20 | adult_count_cmr >= 20) ~ "3b-recovering",
      res_class == "3-post" & !(is.na(adult_count_visual) & is.na(adult_count_cmr)) ~ "3a-persisting",
      .default = NA_character_
    )) %>%
  group_by(population) %>%
  mutate(res_class_post = zoo::na.locf(res_class_post, na.rm = FALSE),
         res_class_post = zoo::na.locf(res_class_post, fromLast = TRUE, na.rm = FALSE)) %>%
  ungroup()
...
ggplot(pop_year_class, aes(x = year)) +
  ...
  geom_tile(data = pop_year_class,
            aes(y = -0.5, fill = res_class), height = 0.6) +
  geom_tile(data = pop_year_class,
            aes(y = -1, fill = res_class_post), height = 0.6) +
  facet_wrap(population ~ .) +
  ...
  geom_hline(yintercept = log(60), linetype = "dashed", color = "purple") +
  scale_color_manual(values = c("VES" = "orange",
                                "CMR" = "red",
                                "Bd Prev" = "green",
                                "Bd Load" = "blue")) +
  scale_fill_manual(values = c("1-naive" = "#0a0",
                               "2-epizootic" = "#a00",
                               "3-post" = "#00f",
                               "3a-persisting" = "#00f",
                               "3b-recovering" = "#0ff")) +  # Replace with your actual class color keys
  labs(color = "Timeseries", fill = "Residency Class")
