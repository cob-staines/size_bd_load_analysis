...
post_release_mask = expand_grid(unique(first_treatment_events$population),
...
pop_year_class = epi_thresholds %>%
  ...
  mutate(res_class_post = zoo::na.locf(res_class_post, fromLast = TRUE, na.rm = FALSE),
         ...
         adult_count_terp = zoo::na.approx(adult_count, na.rm = FALSE),
         adult_bd_prevalence_terp = zoo::na.approx(adult_bd_prevalence, na.rm = FALSE),
         adult_positive_bd_load_median_terp = zoo::na.approx(adult_positive_bd_load_median, na.rm = FALSE),
         adult_bd_load_median_terp = zoo::na.approx(adult_bd_load_median, na.rm = FALSE)) %>%
  ungroup()
...
ggplot(pop_year_class, aes(x = year)) +
  ...
  geom_point(aes(y = log10(adult_bd_load_median + 1), color = "log10(Bd Load all + 1)"), size = psize) +
  geom_line(aes(y = log10(adult_bd_load_median_terp + 1), color = "log10(Bd Load all + 1)")) +
