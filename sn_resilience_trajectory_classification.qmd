...
# setup
librarian::shelf(tidyverse, dbplyr, here, janitor, RPostgres, DBI, RIBBiTR-BII/ribbitrrr, brms, loo, bayesplot, tidybayes, ggplot2, scales)
...
bd_stats = bd_capture_valid %>%
  ...
  summarise(adult_swab_n = n(),
            ...
            adult_bd_load_median = median(bd_mean_its1_copies_per_swab),
            adult_bd_load_mean = mean(bd_mean_its1_copies_per_swab),
            adult_positive_bd_load_geom_mean = exp(mean(log(bd_mean_its1_copies_per_swab[bd_detected]))),
            adult_bd_load_max = max(bd_mean_its1_copies_per_swab),
            .groups = "drop")
...
For each population:
  ...
  - Post-epizootic was sub-classified as:
    - Count < 20 ~ "3a-persisting"
    - Count >= 20 ~ "3b-recovering"
...
ggplot(pop_year_class, aes(x = year)) +
  geom_vline(data = treatment_events_valid %>%
               filter(grepl("release", treatment_type)), aes(xintercept = year), color = "black", linetype = "dashed") +
  geom_vline(data = treatment_events_valid %>%
               filter(grepl("antifungal", treatment_type)), aes(xintercept = year), color = "black", linetype = "solid") +
  ...
  # geom_point(aes(y = log10(adult_positive_bd_load_median + 1), color = "log10(Bd Load + 1)"), size = psize) +
  # geom_line(aes(y = log10(adult_positive_bd_load_median_terp + 1), color = "log10(Bd Load + 1)")) +
  # geom_point(aes(y = log10(adult_bd_load_mean + 1), color = "log10(Bd Load_mean + 1)"), size = psize) +
  # geom_line(aes(y = log10(adult_bd_load_mean_terp + 1), color = "log10(Bd Load_mean + 1)")) +
  ...
  geom_text(data = pop_adult_swab_counts, x = 2010, y = prev_coef, size = 3, aes(label = paste0(adult_swab_n, " swabs"))) +
  geom_tile(data = pop_year_class, aes(y = -1, fill = res_class_post), height = 1) +
  geom_hline(yintercept = 0.5 * prev_coef, linetype = "dashed", color = "#f1b6da") +
  # geom_hline(yintercept = log10(24000), linetype = "dashed", color = "#5b8efd") +
  geom_hline(yintercept = log10(20 + 1), linetype = "dashed", color = "#0571b0") +
  geom_hline(yintercept = log10(21000 + 1), linetype = "dashed", color = "#d01c8b") +
  # geom_hline(yintercept = log10(600000), linetype = "dashed", color = "#000") +
  facet_wrap(population ~ .) +
  scale_color_manual(values = c("Adult count" = "#0571b0",
                                "Bd prevalence" = "#f1b6da",
                                # "log10(Bd Load + 1)" = "#5b8efd",
                                # "log10(Bd Load_mean + 1)" = "#8e5bfd",
                                # "log10(Bd Load max)" = "#000",
                                "geom-mean(Bd+ load)" = "#d01c8b")) +
  scale_fill_manual(values = c("1-naive" = "#b8e186",
                               "2-epizootic" = "#d01c8b",
                               "3a-persisting" = "#92c5de",
                               "3b-recovering" = "#0571b0")) +
  scale_y_continuous(
