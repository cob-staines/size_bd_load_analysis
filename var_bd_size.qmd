...
cor(dat$svl_mm, dat$target_quant_per_swab)

cor(dat$body_mass_g, dat$target_quant_per_swab)


# run lm target_quant_per_swab ~ M + svl_mm * body_mass_g + svl_mm * size per species

mod.fix <- plm(target_quant_per_swab ~ svl_mm, index = c("site"), model = "random", data=dat)

mod <- plm(target_quant_per_swab ~ svl_mm, index = c("site"), model = "within", data=dat)
