...
infec_size <- raw_data %>% 
  ...
  summarise(n = n()) %>% 
  filter(n > 10) %>% 
  filter(!species_capture %in% c("notophthalmus_viridescens")) #remove toad and newt
...
for (i in 1:length(species_list)) {
  ...
  p <- ggplot(newdata) + 
   ...
   xlab("Snout Vent Length (mm)") + ylab("Infection Intensity") + theme_classic() +
   coord_cartesian(ylim = c(0,max(newdata$upper_se) + 5)) + ggtitle(paste(species_list[i], ", slope:", summary(mod)$coefficients[2,1]))
