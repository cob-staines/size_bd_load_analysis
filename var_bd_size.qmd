...
for (i in 1:length(species_list)) {
  ...
  mod <- lm(target_quant_per_swab ~ svl_mm * body_mass_g + svl_mm * size, data=dat)
  ...
  # predict
  #M = seq(min(dat$M), max(dat$M), len=60)
  M = rep(mean(dat$M), len=60) # Mean
  svl_mm = seq(min(dat$svl_mm), max(dat$svl_mm), len=60)
  #svl_mm = rep(mean(dat$svl_mm), len=60) # Mean
  body_mass_g = seq(min(dat$body_mass_g), max(dat$body_mass_g), len=60)
  #body_mass_g = rep(mean(dat$body_mass_g), len=60) # Mean
  
  size = as.data.frame(seq(min(dat$svl_mm), max(dat$svl_mm), len=60)) %>% 
    mutate(size = case_when(svl_mm < quantile(svl_mm, 0.33) ~ "small",
                            svl_mm > quantile(svl_mm, 0.66) ~ "large",
                            between(svl_mm, 
                                    quantile(svl_mm, 0.33),
  ...
  # make GAM plot
  p <- ggplot(newdata) + 
   geom_point(data=dat, aes(x=svl_mm, y=target_quant_per_swab, color = size), size=2, alpha = 0.2) +
   geom_line(aes(x=svl_mm, y=pred, color = size)) +
   ...
   ggtitle(paste(species_list[21], ", slope:", summary(mod)$coefficients[2,1], 
