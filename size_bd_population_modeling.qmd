...
m1lp <- brm(
  formula = bf(
    # Mean structure: intercept + life stage effect + population random effects
    bd_load_int ~ 1 + life_stage + (1 | population),
    # Zero-inflation: separate for each life stage
    zi ~ 1 + life_stage,
    # Shape parameter: separate for each life stage (optional)
    shape ~ 1 + life_stage
  ),
  data = bd_model_sn,  # Your data
  family = zero_inflated_negbinomial(),
  chains = 4,
  iter = 2000,
  warmup = 1000,
...
saveRDS(m1lsp, file = "model_results_znb_lifestage_pop.rds")
m1lsp = readRDS("model_results_znb_lifestage_pop.rds")
