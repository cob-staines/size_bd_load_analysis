...
saveRDS(mbd_full, file = here("panama_resilience_paper/pa_resilience_models/combined_model.rds"))
mbd_full = readRDS(file = here("panama_resilience_paper/pa_resilience_models/combined_model.rds"))
summary(mbd_full)
...
plot_prev_rope_05 = ggplot(prev_rope_df, aes(x = species_a, y = species_b, fill = rope_05)) +
  ...
  labs(title = "Pairwise comparison of Bd prevalence",
       fill = "Proportion in\nROPE (± 0.05)",
       x = "Species",
       y = "Species",
       caption = "* ~ < 0.05 \n** ~ < 0.01 \n*** ~ < 0.001") +
...
color_vals <- setNames(
  ifelse(all_taxa %in% highlight_taxa,
         c("#E8593C", "#3B8BD4")[match(all_taxa[all_taxa %in% highlight_taxa], highlight_taxa)][
...
color_vals <- setNames(
  sapply(all_taxa, function(x) {
    if (x == highlight_taxa[1]) "#E8593C"
    else if (x == highlight_taxa[2]) "#3B8BD4"
    else "grey50"
  }),
  all_taxa
)
