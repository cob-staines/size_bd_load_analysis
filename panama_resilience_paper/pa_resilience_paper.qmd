...
# setup
librarian::shelf(tidyverse, dbplyr, here, janitor, RPostgres, DBI, RIBBiTR-BII/ribbitrrr, brms, tidybayes, ggplot2, emmeans, ggrepel)
...
Species were included if at least 30 individuals with Bd swabs were present for that species.
...
bd_species_valid = bd_population %>%
  ...
  summarise(count = n(),
            .groups = "drop") %>%
  filter(count >= 30,
         taxon_capture != "silverstoneia_nubicola") %>%
  arrange(desc(count)) %>%
  collect()
...
## by region
```{r}
region_species = c("lithobates_warszewitschii",
                   "colostethus_panamansis")

bd_model_plot = bd_model %>%
         mutate(species = gsub(" ", "\n", get_species_name(taxon_capture)),
                region = str_to_title(gsub("_", " ", region)))

bd_region = bd_model_plot %>%
  filter(taxon_capture %in% region_species) %>%
  group_by(taxon_capture, region) %>%
    summarise(n = n(),
              n_pos = sum(bd_load > 0),
              bd_prevalence = sum(bd_load > 0) / n(),
...
species_cols <- c("Colostethus\npanamansis"    = "#33a02c",
                  "Lithobates\nwarszewitschii" = "#1f78b4")

plot_prev_region = ggplot(bd_region, aes(y = bd_prevalence, x = species,
                                         color = species, shape = region,
                                         group = region)) +
  ...
  scale_shape_manual(values = c(16, 17, 15, 18, 8))  # circle, triangle, square, diamond

ggsave(filename = here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_prev_obs_adult_region.png"),
       plot = plot_prev_region,
       height = 4,
       width = 6)
...
# load
plot_load_region = ggplot(bd_model_plot %>%
                            filter(log_bd_load > 0,
                                   taxon_capture %in% region_species), aes(log_bd_load, x = species, color = region)) +
  geom_boxplot() + 
  theme_minimal() +
  # ggtitle("Observed Bd-positive load") +
  xlab("Species") +
  ...
  #   aes(
  #     x = species,
  #     y = max(bd_model_plot$log_bd_load) * 1.05,
  #     label = paste0("n=", n_pos)
  #   ),
...
ggsave(filename = here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_load_obs_adult_region.png"),
       plot = plot_load_region,
       height = 4,
       width = 6)
...
saveRDS(mbd_full, file = here(paste0("panama_resilience_paper/pa_resilience_models/combined_model_", today(), ".rds")))
mbd_full = readRDS(file = here("panama_resilience_paper/pa_resilience_models/combined_model_2026-09-22.rds"))
summary(mbd_full)
...
prev_draws = mbd_full %>%
  spread_draws(
    b_hu_Intercept,
    b_hu_taxon_capturecolostethus_panamansis,
    b_hu_taxon_capturecraugastor_crassidigitus, 
    b_hu_taxon_capturecraugastor_fitzingeri,
    b_hu_taxon_capturedendrobates_auratus, 
    b_hu_taxon_captureespadarana_prosoblepon,
    b_hu_taxon_capturelithobates_warszewitschii, 
    b_hu_taxon_capturepristimantis_ridens,
    b_hu_taxon_capturerhaebo_haematiticus, 
    b_hu_taxon_capturerhinella_marina,
    b_hu_taxon_capturesachatamia_albomaculata, 
    b_hu_taxon_capturesilverstoneia_flotator,
    b_hu_taxon_capturesmilisca_sila) %>%
  mutate(b_hu_taxon_captureatelopus_varius = 0) %>%
  pivot_longer(c("b_hu_taxon_captureatelopus_varius",
                 "b_hu_taxon_capturecolostethus_panamansis",
                 "b_hu_taxon_capturecraugastor_crassidigitus", 
                 "b_hu_taxon_capturecraugastor_fitzingeri",
                 "b_hu_taxon_capturedendrobates_auratus", 
  ...
  mutate(full_intercept = b_hu_Intercept + taxon_intercept,
         bd_prevalence = plogis(1 - full_intercept),
         taxon_capture = gsub("b_hu_taxon_capture", "", param))

prev_draws_summary = prev_draws %>%
  group_by(taxon_capture) %>%
  summarize(prevalence_mean = mean(bd_prevalence),
            prevalence_median = median(bd_prevalence),
            prevalence_ci05 = quantile(bd_prevalence, 0.05),
            prevalence_ci25 = quantile(bd_prevalence, 0.25),
            prevalence_ci75 = quantile(bd_prevalence, 0.75),
            prevalence_ci95 = quantile(bd_prevalence, 0.95),
            .groups = "drop") %>%
  mutate(species = get_species_name(taxon_capture))

emm_prev = emmeans(mbd_full, ~ taxon_capture, type = "response")
emm_prev
...
prev_rope_df =  as.data.frame(t(combn(taxon_list_prev, 2))) %>%
  ...
  mutate(rope_05 = map2_dbl(taxon_a,
         ...
         species_a = factor(get_species_name(taxon_a), levels = unlist(lapply(taxon_list_prev, get_species_name))),
         species_b = factor(get_species_name(taxon_b), levels = unlist(lapply(taxon_list_prev, get_species_name))),
...
#   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
# 
# ggsave(filename = here("pa_resilience_paper_files", "pa_bd_prev_draws_adult.png"),
#        plot = plot_prev_draws,
#        height = 6,
#        width = 8)
# 
# plot_prev_draws
prev_draws_summary_plot = prev_draws_summary %>%
  mutate(species = factor(species, levels = prev_draws_summary %>%
                            arrange(desc(prevalence_median)) %>%
                            # arrange(species) %>%
                            pull(species)))

# draws summary
highlight_cols <- c("Colostethus panamansis"    = "#33a02c",
                    "Lithobates warszewitschii" = "#1f78b4")

plot_prev_draws_sum = ggplot(prev_draws_summary_plot,
                             aes(x = species, y = prevalence_mean,
                                 ymin = prevalence_ci05, ymax = prevalence_ci95,
                                 color = species)) +
  geom_pointrange() +
  scale_color_manual(values = highlight_cols, na.value = "black", guide = "none") +
  labs(y = "Bd prevalence\n(posterior mean ± 95% CrI)",
       x = "Species") +
  ylim(0, 1) +
  theme_classic() +
...
ggsave(filename = here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_prev_draws_summary_adult.png"),
       plot = plot_prev_draws_sum,
       height = 5,
       width = 7)
...
#   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
#
# ggsave(filename = here("pa_resilience_paper_files", "pa_bd_prev_emm_adult.png"),
#        plot = plot_prev_emm,
#        height = 6,
#        width = 8)
# 
...
# rope
plot_prev_rope_05 = ggplot(prev_rope_df, aes(x = species_a, y = species_b, fill = rope_05)) +
  ...
  labs(
    # title = "Pairwise comparison of Bd prevalence",
       fill = "Proportion in\nROPE (± 0.05)",
       x = "Species",
       y = "Species",
       caption = "* ~ < 0.05 \n** ~ < 0.01 \n*** ~ < 0.001") +
  theme(plot.caption = element_text(hjust = 1.4, vjust = 75))

ggsave(filename = here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_prev_rope_05_adult.png"),
...
load_draws_summary = load_draws %>%
  group_by(taxon_capture) %>%
  summarize(log_load_mean = mean(bd_log_load),
            log_load_median = median(bd_log_load),
            log_load_ci05 = quantile(bd_log_load, 0.05),
            log_load_ci25 = quantile(bd_log_load, 0.25),
            log_load_ci75 = quantile(bd_log_load, 0.75),
            log_load_ci95 = quantile(bd_log_load, 0.95),
            .groups = "drop") %>%
  mutate(species = get_species_name(taxon_capture))

# emmeans
emm_load = emmeans(mbd_full, ~ taxon_capture, type = "response")
emm_load
...
load_rope_df = as.data.frame(t(combn(taxon_list_load, 2))) %>%
  ...
  mutate(rope_1 = map2_dbl(taxon_a,
         ...
         species_a = factor(get_species_name(taxon_a), levels = unlist(lapply(taxon_list_load, get_species_name))),
         species_b = factor(get_species_name(taxon_b), levels = unlist(lapply(taxon_list_load, get_species_name))),
...
#   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
# 
# ggsave(filename = here("pa_resilience_paper_files", "pa_bd_load_draws_adult.png"),
#        plot = plot_load_draws,
#        height = 6,
#        width = 8)
# 
# plot_load_draws
load_draws_summary_plot = load_draws_summary %>%
  mutate(species = factor(species, levels = load_draws_summary %>%
                            # arrange(desc(log_load_median)) %>%
                            arrange(desc(species)) %>%
                            pull(species)))

# draws summary
plot_load_draws_sum = ggplot(load_draws_summary_plot, aes(x = species, y = log_load_mean, ymin = log_load_ci05, ymax = log_load_ci95)) +
  geom_pointrange() +
...
ggsave(filename = here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_load_draws_summary_adult.png"),
       plot = plot_load_draws_sum,
       height = 6,
       width = 8)

draws_summary_table = prev_draws_summary_plot %>%
  full_join(load_draws_summary_plot, by = c("species", "taxon_capture")) %>%
  select(-taxon_capture) %>%
  select(species,
         everything())

write_csv(load_draws_summary_plot, here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_adult_draws_summary.csv"))
...
#   theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
# 
# ggsave(filename = here("pa_resilience_paper_files", "pa_bd_load_emm_adult.png"),
#        plot = plot_load_emm,
#        height = 6,
#        width = 8)
# 
...
# rope
plot_load_rope_1 = ggplot(load_rope_df, aes(x = species_a, y = species_b, fill = rope_1)) +
  geom_tile(color = "black") +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, face = "italic"),
        axis.text.y = element_text(face = "italic")) +
  geom_text(aes(label = sig_txt), color = "white", size = 4) +
    labs(
      # title = "Pairwise comparison of Bd prevalence",
       ...
       y = "Species",
       caption = "* ~ < 0.05 \n** ~ < 0.01 \n*** ~ < 0.001") +
  theme(plot.caption = element_text(hjust = 1.2, vjust = 75))

ggsave(filename = here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_load_rope_1_adult.png"),
...
newdata_species <- data.frame(taxon_capture = species_levels)
 
# extract draws with linpred
mu_draws    <- posterior_linpred(mbd_full, newdata = newdata_species,
                                  dpar = "mu", transform = TRUE, re_formula = NA)
hu_draws    <- posterior_linpred(mbd_full, newdata = newdata_species,
                                  dpar = "hu",    transform = TRUE, re_formula = NA)
sigma_draws <- posterior_linpred(mbd_full, newdata = newdata_species,
                                  dpar = "sigma", transform = TRUE, re_formula = NA)
 
colnames(mu_draws)    <- species_levels
colnames(hu_draws)    <- species_levels
colnames(sigma_draws) <- species_levels
 
n_draws <- nrow(mu_draws)
 
# build resample grid
pos_loads <- bd_model$bd_load[bd_model$bd_load > 0]
log_grid_resolution <- 200   # <-- adjust resolution here
log_grid <- seq(
  from = 0,   # small padding below observed range
  to   = log(max(pos_loads)) + 0.5,   # small padding above observed range
...
for (sp in species_levels) {
  ...
  mu_d    <- mu_draws[, sp]      # length n_draws
  sigma_d <- sigma_draws[, sp]   # length n_draws
  hu_d    <- hu_draws[, sp]      # length n_draws
 
  ## outer-product style: (n_draws x n_grid) matrix of z-scores
  z_mat <- outer(log_grid, mu_d, FUN = function(x, mu) x - mu) # n_grid x n_draws
  z_mat <- t(z_mat) / sigma_d                                   # n_draws x n_grid (sigma broadcast per row)
 
  surv_lognormal <- 1 - pnorm(z_mat)        # P(log_load > x | non-structural-zero), n_draws x n_grid
  surv_total <- (1 - hu_d) * surv_lognormal # multiply hu (n_draws) down each row
 
  ## quantiles across draws (rows), per grid column
...
ccdf_df <- bind_rows(ccdf_list) %>%
  mutate(species = str_to_sentence(gsub("_", " ", taxon_capture)),
         log10_load = log_load / log(10))
 
# plot
p <- ggplot(ccdf_df, aes(x = log_load, y = median)) +
  geom_ribbon(aes(ymin = q25, ymax = q75, fill = taxon_capture), alpha = 0.25) +
  geom_line(aes(color = taxon_capture), linewidth = 0.9) +
  labs(
    x = "log(Bd load)",
...
# Define your two highlighted species
highlight_taxa <- c("Colostethus panamansis", "Lithobates warszewitschii")

ccdf_df_levels = ccdf_df %>%
  filter(log_load == 0) %>%
  arrange(median) %>%
  pull(species)

ccdf_df$species = factor(ccdf_df$species, levels = ccdf_df_levels)

# Build a named color vector: highlighted taxa get distinct colors, rest get grey
all_taxa <- unique(ccdf_df$species)

color_vals <- setNames(
  ifelse(all_taxa %in% highlight_taxa,
         c("#33a02c", "#1f78b4")[match(all_taxa[all_taxa %in% highlight_taxa], highlight_taxa)][
           match(all_taxa, all_taxa[all_taxa %in% highlight_taxa])
         ],
         "grey70"),
  all_taxa
)

# Simpler approach using case_when in a named vector:
color_vals <- setNames(
  sapply(all_taxa, function(x) {
    if (x == highlight_taxa[1]) "#33a02c"
    else if (x == highlight_taxa[2]) "#1f78b4"
    else "grey50"
  }),
  all_taxa
)
...
label_df = ccdf_df %>%
  mutate(order_num = 13 - as.integer(factor(species, levels = ccdf_df_levels))) %>%
  filter(log10_load < (5.1 + order_num * 0.05) / log(10)) %>%   # was in ln units
  group_by(species) %>%
  slice_max(log10_load) %>%
  ungroup()

p_g <- ggplot(ccdf_df, aes(x = log10_load, y = median)) +
  geom_ribbon(aes(ymin = q25, ymax = q75, fill = species), alpha = 0.25) +
  geom_line(aes(color = species)) +
  scale_color_manual(values = color_vals) +
  scale_fill_manual(values = fill_vals) +
  labs(
    x = expression(log[10]*"(Bd load)"),
    y = "Median P(Bd load > x)\nwith 25-75% posterior envelope"
  ) +
  theme_classic() +
  theme(legend.position = "none") +
  geom_text_repel(
    data = label_df,
    aes(x = log10_load, y = median, label = species, color = species),
    size = 3.5,
    fontface = "italic",
    nudge_x = 5 / log(10),        # ~2.17, same visual offset as before
    nudge_y = 0.14,
    direction = "y",
    hjust = 0,
    segment.size = 0.3,
...
ggsave(filename = here("panama_resilience_paper", "pa_resilience_paper_files", "pa_bd_ccdf_grey.png"),
       plot = p_g,
       height = 4,
       width = 6,
       device = ragg::agg_png)
