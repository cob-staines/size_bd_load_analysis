...
saveRDS(brm_prev, file = here("panama_resilience_paper/pa_resilience_models/prev_model.rds"))
...
saveRDS(brm_prev_null, file = here("panama_resilience_paper/pa_resilience_models/prev_model_null.rds"))
...
# draws summary
plot_prev_draws_sum = ggplot(prev_draws_summary, aes(x = species, y = prevalence_mean, ymin = prevalence_ci05, ymax = prevalence_ci95)) +
  geom_pointrange() +
...
saveRDS(brm_load, file = here("panama_resilience_paper/pa_resilience_models/load_model.rds"))
...
saveRDS(brm_load_null, file = here("panama_resilience_paper/pa_resilience_models/load_model_null.rds"))
...
label_df <- ccdf_df %>%
  group_by(taxon_capture) %>%
  slice_min(log_load, n = 1) %>%   # one row per species: its rightmost point
  ungroup() %>%
  mutate(species = str_to_sentence(gsub("_", "\n", taxon_capture)))


p_overlay <- ggplot(ccdf_df, aes(x = log_load, y = median, color = taxon_capture)) +
  geom_ribbon(aes(ymin = q25, ymax = q75, fill = taxon_capture), alpha = 0.15, color = NA) +
  geom_line(linewidth = 0.9) +
  geom_text_repel(
    data = label_df,
    aes(x = log_load, y = median, label = species, color = taxon_capture),
    # size = 3,
    # fontface = "italic",
    # direction = "y",
    # hjust = 0,
    # nudge_x = 0.3,          # push labels right of their anchor point
    # segment.size = 0.3,
    # max.overlaps = Inf,
    show.legend = FALSE
  ) +
  ...
  theme_minimal(base_size = 12) +
  theme(legend.position = "none") +
  coord_cartesian(clip = "off")   # so labels nudged outside the panel aren't cut off
