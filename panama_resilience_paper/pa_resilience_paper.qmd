...
plot_prev_region = ggplot(bd_region, aes(y = bd_prevalence, x = species, color = region)) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = bd_prevalence_ci_lower, ymax = bd_prevalence_ci_upper),
                width = 0.2,
                position = position_dodge(width = 0.5)) +
...
# draws summary
plot_prev_draws_sum = ggplot(prev_draws_summary_plot, aes(x = species, y = prevalence_mean, ymin = prevalence_ci05, ymax = prevalence_ci95)) +
  geom_pointrange() +
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, face = "italic"),
        plot.margin = margin(t = 5, r = 5, b = 5, l = 40))  # increase l until the label fits)
