---
title: "Size vs. Bd modeling: SIerra Nevada preliminary results"
author: Cob Staines
date: 2025-09-05
format:
  html:
...
Populations comprised of sites grouped to the "population" level (where gene flow between sites is indicated or expected on the scale of a year), with the following selection criteria used to determine which species to include in the analysis:
  ...
  4. at least 15 adult and subadult each per population
...
m_zln_ramu = load_model(taxon_m, template = "model_results_zln_life_tax_pop_xxxx_all.rds")
...
taxon_m = "rana_sierrae"
spec_name = get_species_name(taxon_m)
m_zln_rasi = load_model(taxon_m, template = "model_results_zln_life_tax_pop_xxxx_all.rds")
