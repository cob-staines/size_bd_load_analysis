...
In some sites where MYLF populations have persisted over the years despite Bd, we have the opportunity to investigate how Bd infection prevalence and loads differ between adults and subadults through time.

## Working hypotheses:

  a. In species with greater Bd load in younger individuals, we expect subadult Bd load to decrease in time compared with adults (due to specific evolutionary forcing at this point in the life cycle)
  b. In species without greater Bd load in younger individuals, we expect subadult Bd load to trend with adult Bd load in time
...
Populations comprised of sites grouped to the "region" level, with the following selection criteria used to determine which species to include in the analysis:
  ...
  2. populations sampled over at least 15 years
  3. at least 15 adult and subadult each per population
...
bd_population = bd_sample %>%
  left_join(db_bd %>%
              ...
              summarise(bd_swab_count = n(),
                        bd_detected = any(bd_its1_copies_per_swab > 0),
...
# pull all data meeting criteria
...
# # when did Bd first show up in a basin (at least in these data)?
# bd_basin = bd_model %>%
#   group_by(taxon_capture, geographic_area) %>%
#   summarise(n = n(),
#             first_year_sample = min(year),
#             first_year_bdpos = min(year[bd_load > 0]),
#             .groups = "drop") %>%
#   arrange(taxon_capture,
#           geographic_area)

pops_by_year = bd_model %>%
  group_by(population, year, life_stage_simple) %>%
  summarise(n = n(),
            bd_pos = any(bd_load > 0),
            .groups = "drop") %>%
  group_by(population) %>%
  mutate(year_first_bd = min(year[bd_pos]),
            .groups = "drop") %>%
  filter(n >0)

ggplot(pops_by_year, aes(x = year, y = n, color = life_stage_simple)) +
  geom_point() + 
  geom_line() +
  geom_vline(aes(xintercept = year_first_bd), color = "orange") +
