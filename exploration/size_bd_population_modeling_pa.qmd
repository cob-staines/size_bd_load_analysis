...
Species were were included for a given season if at least 14 adult AND subadult each were present for that species and season. *Atelopus varius* was also included in the analysis despite not meeting this criteria, in the interest of discussing the applicability of what we learn from more "data-rich" species to conservation focused on "data-poor" species such as *A. varius*.
...
bd_species_valid = bd_population %>%
  ...
  group_by(taxon_capture, season, life_stage_simple) %>%
  summarise(count = n(),
            count_gte_14 = n() >= 14,
            .groups = "drop") %>%
  group_by(taxon_capture, season) %>%
  filter(sum(as.integer(count_gte_14)) == 2 | (taxon_capture == "atelopus_varius" & season == "lluvia")) %>%
  ungroup() %>%
  collect()
...
bd_sample_valid = bd_population %>%
  ...
  inner_join(bd_species_valid %>%
               select("taxon_capture",
                      "season",
                      "life_stage_simple"), by = c("taxon_capture", "season", "life_stage_simple"), copy = TRUE) %>%
  inner_join(db_bd %>%
...
avar = bd_model %>%
  ...
  arrange(population, site, date)

# write_csv(avar, "size_bd_a_varius.csv")
...
taxon_m = "rhinella_marina"
output_m = "model_results_zln_life_tax_sea_pop_rhma.rds"
m2_ltp = run_m2_ltp_1s(taxon_m)
saveRDS(m2_ltp, file = output_m)
...
simulate_bd_load_fixed = function(draws, n_per_draw = 10, downsample = NA, species_name) {
  if (!is.na(downsample) & downsample < nrow(draws)) {
    draws = draws %>%
      group_by(life_stage_simple, season) %>%
  ...
  draws %>%
    ...
    ungroup() %>%
    mutate(taxon_capture = species_name)
}
...
sim_load_atva = simulate_bd_load_fixed(draws_atva, 10, 1000, spec_name)
...
sim_load_copa = simulate_bd_load_fixed(draws_copa, 10, 1000, spec_name)
...
sim_load_crcr = simulate_bd_load_fixed(draws_crcr, 10, 1000, spec_name)
...
sim_load_crfi = simulate_bd_load_fixed(draws_crfi, 10, 1000, spec_name)
...
sim_load_espr = simulate_bd_load_fixed(draws_espr, 10, 1000, spec_name)
...
sim_load_liwa = simulate_bd_load_fixed(draws_liwa, 10, 1000, spec_name)
...
sim_load_rhha = simulate_bd_load_fixed(draws_rhha, 10, 1000, spec_name)
...
#| output: false
#| code-fold: true
#| code-summary: "Show Rhinella marina code"

taxon_m = "rhinella_marina"
spec_name = get_species_name(taxon_m)
m_zln_rhma = load_model(taxon_m)

draws_rhma = collect_draws_1s(m_zln_rhma) %>%
  mutate(taxon_capture = spec_name,
         season = "lluvia")

density_mat_rhma = density_mat(log(0.3), 20, 100, draws_rhma) %>%
  mutate(taxon_capture = spec_name)

# pps_rhma = sample_posterior_life_sea(m_zln_rhma, spec_name, 10000)

sim_load_rhma = simulate_bd_load_fixed(draws_rhma, 10, 1000, spec_name)

bd_model_rhma = bd_model %>%
  filter(taxon_capture == taxon_m,
         !is.na(season))

obs_stats_rhma = sample_stats(bd_model_rhma) %>%
  mutate(src = "obs",
         taxon_capture = spec_name)

# summary(m_zln_rhma)
# model_report(m_zln_rhma)
```
::: {.panel-tabset}
### Observed Prevalence
```{r}
#| echo: false
#| warning: false
(plot_1 = plot_prev_obs(obs_stats_rhma, spec_name))
```
### Modeled Prevalence 
```{r}
#| echo: false
#| warning: false
(plot_2 = plot_prev_mod(draws_rhma, spec_name))
```
### Observed Bd load
```{r}
#| echo: false
#| warning: false
(plot_3 = plot_load_obs(bd_model_rhma, obs_stats_rhma, spec_name))
```
### Modeled Bd load
```{r}
#| echo: false
#| warning: false
(plot_4 = plot_load_mod(sim_load_rhma, spec_name))
```
### Modeled Bd load PDF
```{r}
#| echo: false
#| warning: false
(plot_5 = plot_pdf(density_mat_rhma, spec_name))
```
### Moded Bd load CCDF
```{r}
#| echo: false
#| warning: false
(plot_6 = plot_ccdf(density_mat_rhma, spec_name))
```
:::
...
sim_load_saal = simulate_bd_load_fixed(draws_saal, 10, 1000, spec_name)
...
sim_load_sifl = simulate_bd_load_fixed(draws_sifl, 10, 1000, spec_name)
...
sim_load_smsi = simulate_bd_load_fixed(draws_smsi, 10, 1000, spec_name)
...
bd_model_n = bd_model_stats %>%
  select(taxon_capture,
         ...
         adult_n,
         subadult_n) %>%
  filter(taxon_capture %in% taxa_season) %>%
