...
pairwise_comparison = pairs(emm_prev, adjust = "tukey")
pairwise_comparison

emm_df = as.data.frame(emm_prev)
...
prev_rope_df = cross_join(taxon_list, taxon_list) %>%
...
# draws
ggplot(prev_draws, aes(x = taxon_capture, y = bd_prevalence)) +
  geom_boxplot() +
...
# emmeans
ggplot(emm_df, aes(x = taxon_capture, y = response, ymin = lower.HPD, ymax = upper.HPD)) +
  geom_pointrange() +
...
# rope %>%
ggplot(rope_df, aes(x = taxon_a, y = taxon_b, fill = rope_05_05)) +
...
load_rope_df = cross_join(taxon_list, taxon_list) %>%
  rename(taxon_a = taxon_capture.x,
         taxon_b = taxon_capture.y) %>%
  mutate(rope_05 = map2_dbl(taxon_a,
                            taxon_b,
                            ~ f_rope(load_draws, "bd_log_load", .x, .y, epsilon = 0.05, factor = 5)),
         rope_05_05 = rope_05 < 0.05
  )
...
# draws
ggplot(load_draws, aes(x = taxon_capture, y = bd_log_load)) +
  geom_boxplot() +
...
# emmeans
ggplot(emm_load_df, aes(x = taxon_capture, y = emmean, ymin = lower.HPD, ymax = upper.HPD)) +
  geom_pointrange() +
...
# rope %>%
ggplot(load_rope_df, aes(x = taxon_a, y = taxon_b, fill = rope_05_05)) +
  geom_tile(color = "black") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
