...
peace = bd_sample %>%
  filter(taxon_capture == "dendrobates_auratus") %>%
  select(site,
         date,
         taxon_capture,
         sample_name) %>%
...
data_stats = bd_model %>%
  ...
  summarise(n_bd_swabs = n(),
            .groups = "drop") %>%
  write_csv("panama_grid_site_year_species.csv")
...
data_grid_site_year = bd_model%>%
  ...
  arrange(site) %>%
  write_csv("panama_grid_site_year.csv")
...
data_grid_species_year = bd_model%>%
  ...
  pivot_wider(names_from = "year",
              values_from = "n_bd_swabs") %>%
  write_csv("panama_grid_species_year.csv")
