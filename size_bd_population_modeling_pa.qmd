...
sample_posterior_life_sea = function(m_zln, species_name, n = 1000, s = 1000){
  ps_adult_lluvia = samp_pos(m_zln, "adult", "lluvia", n, s)
  ps_adult_seca = samp_pos(m_zln, "adult", "seca", n, s)
  ps_subadult_lluvia = samp_pos(m_zln, "subadult", "lluvia", n, s)
  ps_subadult_seca = samp_pos(m_zln, "subadult", "seca", n, s)
  
  # develop a better way to pass matrix data along with life stage and season
  
  df_samps_adult_lluvia = data.frame(
    life_stage_simple = "adult",
    season = "lluvia",
    bd_load = I(list(ps_adult_lluvia))
  )
  
  df_samps_adult_seca = data.frame(
    life_stage_simple = "adult",
    season = "seca",
    bd_load = I(list(ps_adult_seca))
  )
  df_samps_subadult_lluvia = data.frame(
    life_stage_simple = "subadult",
    season = "lluvia",
    bd_load = I(list(ps_subadult_lluvia))
  )
  df_samps_subadult_seca = data.frame(
    life_stage_simple = "subadult",
    season = "seca",
    bd_load = I(list(ps_subadult_seca))
  )
...
pps_coma = sample_posterior_life_sea(m_zln_coma, spec_name, 1000, 1000)

## bookmark - passing these data on to plotting functions plz
...
peace = draws_coma %>% slice(22)
pdf_grid = function(draw) {
  
}

density_grid <- seq(log(1.3), 20, length.out = 100)
colnames(draws_coma)

grouped_draws <- draws_coma %>%
...
group_summaries <- map(grouped_draws, function(group_df) {
  ...
  # Summary statistics: mean and 95% CI
  median <- apply(density_mat, 2, quantile, probs = 0.5)
  lower_ci <- apply(density_mat, 2, quantile, probs = 0.025)
  upper_ci <- apply(density_mat, 2, quantile, probs = 0.975)
...
ggplot(summary_df, aes(x = density, y = median, color = life_stage_simple, linetype = season)) +
  geom_line() +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3)

ggplot(summary_df, aes(x = density,
                       y = median,
                       color = life_stage_simple, 
                       fill = life_stage_simple,   # add fill mapping
                       linetype = season)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2, color = NA) +  # no borders on ribbons
  geom_line() +
  theme_minimal() +
  xlab("log(1 + Bd load)") +
  ylab("1 - Cumulative probability") +
...
## PDF
ggplot(pps_coma, aes(x = log_bd_load, color = life_stage_simple, linetype = season)) +
  geom_density(kernel="epanechnikov") +
  theme_minimal() +
  ggtitle(paste0("Bd load PDF: ", spec_name)) +
  xlab("log(Bd load + 1) [-]") +
  ylab("Probability density [-]") +
  labs(color = "Life stage",
       linetype = "Season")

linpred_draws <- posterior_linpred(m_zln_coma, transform = TRUE)
